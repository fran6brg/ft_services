...
function vm_start
{
	...
	cp srcs/telegraf/telegraf_template.conf srcs/telegraf/telegraf.conf
	sed -i '' s/$(cat srcs/telegraf/telegraf.conf | grep MINIKUBEIP  | awk '{print $3}' | cut -c9- | rev | cut -c8- | rev)/$(minikube ip)/g srcs/telegraf/telegraf.conf
